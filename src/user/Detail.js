...
import { CartItemAdd, ShopItemFetch } from "../fetches/fetching";
...
const Detail = () => {
  ...
  const handleAdd = async (e) => {
    ...
    CartItemAdd(item[0].item_nm, item[0].item_price, quantity, userId);
    setQuantity(0);
    navigate('/cart');
  }
        ...
        <div className="row m-auto p-5" style={{ textAlign: 'justify' }}>
          ...
          item.length > 0 ?
            <div className="col-lg-7 order-lg-2 d-flex flex-column">
              <h3>{item[0].item_nm}</h3>
              <h5>{item[0].item_price}</h5>
              <p>
                {item[0].item_desc}
              </p>
